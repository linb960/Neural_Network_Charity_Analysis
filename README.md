...
<br>
In addition to using a Neural Networks on this data we might also want to use __Random Forest Classifiers__ because Random forest models use a number of weak learner algorithms (decision trees) and we are ultimately answering a yes or no question about whether an organization is successful.  Also structurally speaking, random forest models are very similar to their neural network counterparts. Random forest models can also easily handle outliers and may be a better choice in this analysis due to the outliers mentioned above.  
